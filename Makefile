...
#    Updated: 2023/08/08 14:47:50 by croy             ###   ########lyon.fr    #
#                                                                              #
# **************************************************************************** #
...
SRC_FOLDER := src/
OBJ_DIR := obj/
SRC = $(addprefix $(SRC_FOLDER), $(SRC_EXEC_FUNCTIONS) $(SRC_EXEC_UTILS) $(SRC_UTILS) $(SCR_PARSING))
OBJ = $(subst $(SRC_FOLDER),$(OBJ_DIR),$(SRC:.c=.o))

DIR_UTILS := $(SRC_FOLDER)utils/
SRC_UTILS := minishell.c exec_char.c

DIR_EXEC_FUNCTIONS := $(SRC_FOLDER)exec/functions/
...
SCR_PARSING := check_in_quotes.c  tokens.c tokens2.c cut_expand.c parsing.c envp.c envp_lst.c replace_list.c tokens_lst.c dollar.c dollar2.c dollar3.c identification.c prepare.c tilde.c rm_quotes.c syntax_error.c signal.c signal2.c
...
$(OBJ_DIR)%.o : $(DIR_EXEC_FUNCTIONS)%.c $(HEADER) | $(OBJ_DIR)
	$(CC) $(CFLAGS) -o $@ -c $<
